...
function updateLeadStatus(conversationId, status) {
  const allowedStatuses = ["new", "pending", "interested", "closed"];
  const conversation = conversationsState.find((item) => item.id === conversationId);
  ...
  if (!allowedStatuses.includes(status)) {
    return { error: "Invalid lead status", conversation: null };
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/") {
    ...
    res.end(
      JSON.stringify({
        ok: true,
        service: "api",
        routes: ["/", "/health", "/conversations", "/channels", "/reply", "/lead-status"],
      })
    );
    return;
  }
  ...
  if (req.url === "/lead-status" && req.method === "POST") {
    try {
      const rawBody = await collectRequestBody(req);
      const body = JSON.parse(rawBody || "{}");
      ...
      if (!body.conversationId || !body.status) {
        res.writeHead(400, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "conversationId and status are required" }));
        return;
      }

      const result = updateLeadStatus(body.conversationId, body.status);
      ...
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ conversation: result.conversation }));
      return;
    } catch (error) {
      res.writeHead(400, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: "Invalid JSON body" }));
